...
${workflowId ? "5. Call anydb_get_workflow and read the stored source at the action_script entry's config.script, plus the latest executionHistory entries and their output.logLines (and, for a failed run, output.error and output.trace, which name the anydb calls it made). Diagnose the actual defect before rewriting; preserve behavior the request does not ask you to change.\n" : ""}

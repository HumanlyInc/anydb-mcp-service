...
export const SOLUTION_AUTHORING_TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description: `Read the authoring guide (call anydb_get_authoring_guide) before the first authoring call in a task. Patch the latest revision of one workspace type by stable template name (pass expectedRevision = the revision from anydb_get_type_definition), including its description, icon, titleFormula, fields, badges, or child policy. changes.titleFormula sets or replaces the record-title formula on an existing type, so there is no need to recreate a type just to change how its records are named. A field's props map carries cell properties such as BACKGROUND_COLOR, TEXT_BOLD, CELL_HIDDEN, and CELL_ERROR, each taking a value, an expr, or both; sending props replaces the whole map for that field, so read the field first with anydb_get_type_definition and resend it with your edit applied, or omit props to leave existing properties untouched. To add values to an existing select's list use changes.appendOptions ([{ key, options }]): the new options go on the end, existing ones stay, and it is never destructive; updateFields with options replaces the whole list. Existing records are migrated automatically after persistence; destructive changes require explicit data-loss confirmation.`,
  ...
  },
  {
    ...
    description:
      "Execute an existing workflow, by workflowId or by workflowName. Set simulate=true to dry-run without side effects, or simulate=false for normal execution. Supply adoid when the workflow requires record context. READ result.executionRan BEFORE ANYTHING ELSE: when it is false the workflow ran and its trigger correctly declined to act on that record, which is a success and not an error, and execution is null deliberately rather than missing - no history is written for a run that did not fire, so do not retry, do not report the automation as broken, and re-run against data that satisfies the trigger's conditions instead. When executionRan is true, inspect the returned execution for artifact outputs and errors. An older server omits the field entirely; treat a present execution as a run. THIS IS ALSO HOW YOU PRESS A BUTTON. A button-format cell stores BUTTON_ACTION_TYPE 'automation' and BUTTON_ACTION_VALUE, and BUTTON_ACTION_VALUE is the workflow's NAME, not its id - so read the button cell off the record with get_record and pass its BUTTON_ACTION_VALUE as workflowName, along with that record's adoid. There is no button-specific trigger; a button runs an ordinary workflow. This reaches the same server code a real click reaches, with the same record permissions and the same attribution to you as the caller. One difference: CELL_LOCKED and CELL_HIDDEN on a button only grey it out in the app, and are not checked here, so a button a person could not press can still be fired this way - check the cell's props yourself if that matters.",

...
export const SOLUTION_AUTHORING_TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description: `Read the authoring guide (call anydb_get_authoring_guide) before the first authoring call in a task. Patch the latest revision of one workspace type by stable template name, including its description, icon, titleFormula, fields, badges, or child policy. changes.titleFormula sets or replaces the record-title formula on an existing type, so there is no need to recreate a type just to change how its records are named. A field's props map carries cell properties such as BACKGROUND_COLOR, TEXT_BOLD, CELL_HIDDEN, and CELL_ERROR, each taking a value, an expr, or both; sending props replaces the whole map for that field, so read the field first with anydb_get_type_definition and resend it with your edit applied, or omit props to leave existing properties untouched. Existing records are migrated automatically after persistence; destructive changes require explicit data-loss confirmation.`,

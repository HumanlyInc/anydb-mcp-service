...
const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    description:
      "Set up a Document Generation template: attach an uploaded .docx or .xlsx file to a workspace type, so documents can be generated from records of that type. THE FILE MUST BE UPLOADED FIRST and the upload must be COMPLETED - use prepare_file_upload, PUT the bytes, then complete_file_upload, and pass the resulting File record's adoid as fileRecordId. A record that is not a File, or a File whose upload never completed, is rejected: an agent that skips complete_file_upload has an adoid that looks usable and is not. The type must already exist.\n\nWHAT TO PUT INSIDE THE TEMPLATE FILE - the merge-tag syntax:\n- A placeholder is SINGLE braces around a field name: {Invoice Number}. This is NOT the double-brace {{...}} of AnyDB's formula language - the two look alike and are different. Double braces do also work, because the renderer retries with {{ }} when single-brace parsing reports duplicate delimiters, but single braces are the primary form and the one to write.\n- A field can be named EITHER by its key OR by its cell position: {Invoice Number} and {B2} both resolve to the same cell. Keys are clearer and survive layout changes; positions are useful for cells with no key.\n- A tag naming nothing on the record RENDERS EMPTY. It is not an error and nothing warns you, so a typo in a tag shows up as a blank space in the finished document rather than a failure. Check field names against anydb_get_type_definition before uploading.\n- An attachments cell (child records) can be repeated with a loop: {#Line Items} ... {/Line Items}. Inside the loop each child exposes its own field keys, plus name, adoid, templateName, created and updated. Keep loop tags on rows that also carry visible content; a blank spacer row inside a loop is rejected.\n- If you are BUILDING the .docx yourself rather than saving one from Word, the package must contain word/_rels/document.xml.rels. Word always writes it; a hand-built zip usually omits it, and older servers reject such a file with an unhelpful error.",
    inputSchema: {
      type: "object",
      properties: {
        teamid: { type: "string", description: "The team ID." },
  ...
  },
  {
    ...
    description:
      "Run a short server-side script against a database and get back only what it computes. THIS IS THE ESCAPE HATCH FOR BULK AND AGGREGATE WORK - reach for it when the per-record tools would be the wrong instrument, not for ordinary work on a handful of records. Two cases it exists for: (1) a computation over more records than can be read back one by one, such as an NPS score across tens of thousands of responses, where the script reduces the rows server-side and returns one number; (2) a read-modify-write across many records, where fetching each record and sending it back would mean an agent retyping every payload; (3) loading a set of records - sample data, a seed, an import from rows you generate - with anydb.createRecord in a loop (children by parentid, refs with await created.setCellRefValue). The script runs sandboxed, as YOU, with exactly your permissions - it cannot see or change anything you could not, and it needs no credential beyond this connection: never ask the user for an API key, a token or an external script for work this runtime can do as them. A run is capped at 5 minutes, so slice a large load across runs (a few hundred creates per run) and keep the ids you made in memory between them. Requires the Business or Enterprise plan. YOU MUST SIMULATE FIRST: this tool refuses to run without the runToken that anydb_simulate_script returns for that exact script text. Return values come back through output.set(key, value); output.json/table/csv/markdown instead render a report file. EVERY LOOP BODY MUST CONTAIN AN await (use `await anydb.yield()`), or the script is rejected. READING MANY RECORDS: never fetch a whole large type in one call - it outlasts the call. Narrow with anydb.findRecordsPage({ typeName, condition, limit, cursor }), where condition compares cell keys with == (a single = is not a comparison), or anydb.getChildren(parentid) for one parent's records; page with limit (a few hundred) and pass the returned nextCursor back as cursor, across calls if needed, keeping what you need in the output. Reads cost roughly a second per ten records today, so size pages to finish well inside the limit. There is no parentId or filter argument. Call anydb_list_workflow_actions for the full runtime API available to the script.",
    inputSchema: {
      type: "object",
      properties: {
        teamid: { type: "string", description: "The team ID." },
  ...
  },
  {
    ...
    description:
      "Create up to 100 records in one request. Processing uses bounded concurrency and returns an ordered result for every input; failures do not roll back successful records. Use clientref to correlate results. For rows you generate yourself - sample data, a seed, an import - beyond a few batches, prefer anydb_run_script: it creates them as the connected user in one server-side loop, with no extra credential and no payloads retyped through this tool.",
    inputSchema: {
      type: "object",
      properties: {
        teamid: { type: "string", description: "The team ID" },

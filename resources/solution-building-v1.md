...
- Fetch the set once before the loop and track the records you create in memory, extending that list as you go, rather than re-querying for what this run wrote. Do not retry or poll instead: indexing is not guaranteed to catch up inside the script timeout, so a retry loop burns the budget and still returns nothing.
